...
FILE NAME					: StreamSwitcherVer3.cpp
...
#include "StreamSwitcherVer3.h"

int StreamSwitcherVer3::ResetMembers(void)
{
	/// Reset all members.
	_state				= SSV3_STEADY_STATE;
	_switched			= 1;
	_pCurrStream	= NULL;
	_pNewStream		= NULL;
	...
	_pHeader	= NULL;
	_pHeader	= new SwitchCodecHeaderVer1();
...
StreamSwitcherVer3::StreamSwitcherVer3(void)
{
	/// Reset all members.
	ResetMembers();
}//end constructor.

StreamSwitcherVer3::StreamSwitcherVer3(void* pInitialStream)
{
	/// Reset all members.
	ResetMembers();
	/// Initialise.
...
StreamSwitcherVer3::~StreamSwitcherVer3()
{
	if(_pHeader != NULL)
		delete _pHeader;
	_pHeader = NULL;
...
*/
void* StreamSwitcherVer3::GetNextFrame(int* retBitLen)
{
	if(_pHeader == NULL)
		return(NULL);
				...
				if(_pNewStream != NULL)	///< Indicates entering the switching states.
				{
					...
					else
					{
						...
						if(_pHeader->GetSwitchStreamPicType() == 0)
						{
							...
							_state	= SSV3_WAIT_FOR_SWITCH;
						}//end if GetSwitchStreamPicType...
						else
							_state = SSV3_WAIT_FOR_KEY;
					}//end else...
				}//end if pToStream...
			}//end local block...
			break;
		case SSV3_WAIT_FOR_KEY:
...
*/
int StreamSwitcherVer3::Switch(void* pToStream)
{

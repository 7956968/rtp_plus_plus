...
void writeNalu(const MediaSample& mediaSample, media::h264::NalUnitType eType, std::ofstream& out, bool bExtractBaseLayerOnly)
{
...
int main(int argc, char** argv)
{
  ...
  try
  {
    ...
    // An H.264 stream can be read from file or from stdin
    bool bStdin = false;
    std::ifstream in1;
    ...
    // reason. Moving the declarions after the io_service AND resetting the shared_ptr's
    ...
    if (bStdin)
    {
      pStreamVideoService = makeService<media::StreamMediaSource>(boost::ref(std::cin), rfc6184::H264, 10000, false, 0, uiFps, bLimitRate, eMode);
      if (bAu)
    ...
    {
      pStreamVideoService = makeService<media::StreamMediaSource>(boost::ref(in1), rfc6184::H264, 10000, false, 0, uiFps, bLimitRate, eMode);
      if (bAu)
        pStreamVideoService->get()->setReceiveAccessUnitCB(boost::bind(&handleVideoAccessUnit, _1, boost::ref(out1), bVerbose, bExtractBaseLayer));
      else
        pStreamVideoService->get()->setReceiveMediaCB(boost::bind(&handleVideoSample, _1, boost::ref(out1), bVerbose, bExtractBaseLayer));

#include "CorePch.h"
#define _SCTP_TEST_APP
#ifdef _SCTP_TEST_APP
...
int SctpAssociation::sendUserMessage(size_t id, const char *message, size_t length)
{
  ...
  {
    int err = errno;
    LOG(WARNING) << "sendUserMessage: error on send: " << err;
    if (EWOULDBLOCK == err)
    {
      VLOG(2) << "Try send again";
    }
    perror("sctp_sendv");
    return (0);
  }
...
void SctpAssociation::handle_open_request_message(struct rtcweb_datachannel_open_request *req,
                            size_t length,
                            uint16_t i_stream)
{
...
void SctpAssociation::handle_open_response_message(struct rtcweb_datachannel_open_response *rsp,
...
void SctpAssociation::handle_open_ack_message(struct rtcweb_datachannel_ack *ack,
                        size_t length, uint16_t i_stream)
{
...
void SctpAssociation::handle_message(char *buffer, size_t length, uint32_t ppid, uint16_t i_stream)
{
  boost::mutex::scoped_lock l(m_lock);
...
void SctpAssociation::handle_notification(struct socket *sock, union sctp_notification *notif, size_t n)
{
...
void SctpAssociation::print_status()
{
  boost::mutex::scoped_lock l(m_lock);

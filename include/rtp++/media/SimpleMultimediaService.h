...
/**
 * @brief The SimpleMultimediaService class supports producing/consuming single multimedia sources
 * such as a single video and/or audio stream
 */
class SimpleMultimediaService
{
public:

  /**
   * @brief SimpleMultimediaService Constructor
   */
  SimpleMultimediaService();

  /**
   * @brief SimpleMultimediaService Destructor
   ...
   */
  boost::system::error_code createVideoProducer(boost::shared_ptr<SimpleMediaSession> pMediaSession,
   ...
   * @return boost::system::error_code();
   */
  boost::system::error_code createAudioProducer(boost::shared_ptr<SimpleMediaSession> pMediaSession, uint32_t uiRateMs,
                                                const std::string& sPayloadType, const std::string& sPayloadSizeString,
                                                const uint8_t uiPayloadChar, uint32_t uiTotalSamples,
                                                uint32_t uiDurationMilliSeconds = 0);

...
class PtsBasedJitterBuffer: public IRtpJitterBuffer
{
  // friend class test::RtpJitterBufferTest;
public:
  typedef std::unique_ptr<PtsBasedJitterBuffer> ptr;

  static ptr create(uint32_t uiPlayoutBufferLatency = MAX_LATENCY_MS);
  PtsBasedJitterBuffer(uint32_t uiPlayoutBufferLatency = MAX_LATENCY_MS);
  virtual ~PtsBasedJitterBuffer();
...
protected:
   ...
   * RTP packets that miss their playout point are discarded
   ...
   *         false if it an existing node was updated, or if the RTP packet was late or a duplicate
   */
   ...
   */
  void insertAccordingToPts(const RtpPacket& packet, const boost::posix_time::ptime& tPresentation,
                            bool bRtcpSynchronised,
...
protected:
  ...
  boost::posix_time::ptime m_tFirstPresentationTs;

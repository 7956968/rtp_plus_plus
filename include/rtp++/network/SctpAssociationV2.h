...
struct DataChannelOpenResponse {
...
/**
  ...
  */
static int receiveCallback(struct socket *sock, union sctp_sockstore addr, void *data,
           size_t datalen, struct sctp_rcvinfo rcv, int flags, void *ulp_info);

/**
 * @brief The SctpOptions struct contains parameters that can be set in the SCTP association
 */
struct SctpOptions
{
  SctpOptions()
...
/**
 * @brief The SctpAcceptor class takes care of accepting sockets for server applications
 * It runs in it's own thread since the Sctp
 */
class SctpAcceptor
{
public:
...
public:
   ...
   * @param i_stream
   */
  void handleMessage(char *buffer, size_t length, uint32_t ppid, uint16_t i_stream);

  /**
   * @brief handleNotification
   * @param sock
   * @param notif
   * @param n
   */
  void handleNotification(struct socket *sock, union sctp_notification *notif, size_t n);
...
private:
  ...
  void handle_open_request_message(DataChannelOpenRequest *req,
                              size_t length,
                              uint16_t i_stream);

  void handle_open_response_message(DataChannelOpenResponse *rsp,
                               size_t length, uint16_t i_stream);

  void handle_open_ack_message(DataChannelAck *ack,
                          size_t length, uint16_t i_stream);
